...
* **Zo!** - This is probably the NPC with the easiest ability to hook in the players. Be energetic, animated, and exaggerated. Once the PCs say that they're Starfinders, he immediately starts going into interview mode, instinctively holding a microphone out once he asks the players a question. Encourage the players to improvise the most hilarious stunts that they can do in the gala. Have fun being ridiculous.

### B. The Countdown

This encounter is weird because it's two different encounters happening potentially at the same time. The first encounter is a skill challenge disarming the bombs, the second encounter is a combat with Triloteya.

The flow of this encounter can be really tricky. If the group decides to tackle both challenges, start in "pseudo initiative" - don't have everyone roll initiative, but keep track of rounds and force players to spend only one round of actions at a time, although the players may take their turns in whatever order they want. Doing so will add to the suspense of if they can get the bombs off in time, while also encouraging that they don't just leave the bomb disposal to the person with the highest skill modifier.

There's a couple ways of defeating the skill challenge, and it has multiple parts to keep track of at the same time. Here's a breakdown of everything that is going on.
* **The PCs need to find the bombs** - Each bomb can be found with a DC 20/23 Perception check, and each bomb provides a +2 circumstance bonus to finding the other bombs. Note that searching is a move action, and Starfinder doesn't have the -1/10 ft. penalty that Pathfinder does.
* **The PCs need to disarm the bombs** - There two ways of disarming the bomb. The first is a DC 18/21 Engineering check as a full-round action. The second is doing enough kinetic damage to destroy it (hardness 5/8, HP 15/24). If the Engineering check fails by 5 or more, then only that bomb detonates. If a bomb takes any amount of energy damage, then all remaining bombs detonate.
* **The PCs need to hide their actions from Triloteya** - There are three ways for the PCs to hide their action from Triloteya. The first is that a PC can try to distract Triloteya with a DC 19/22 Bluff, Diplomacy or Intimidate check.
...
{% include spoiler.html id="Two" name="01-09 Live Exploration Extreme!" content="Zo! is looking for people who would be good for the next expedition to Salvation's End, which takes place in this scenario." %}
